...
const targetStatsList = `                            <div className="flex flex-col">
...
const replacementStatsList = `                            <div className="flex flex-col">
                              <span className="text-[10px] text-orange-500 font-black uppercase">{lang === 'es' ? 'Tú' : 'You'}</span>
...
content = content.replace(targetStatsList, replacementStatsList);

fs.writeFileSync('src/TrackerApp.tsx', content, 'utf8');
console.log("Patched stats latest results with decklist links");
