...
const renderEnergyCost = (energy: string) => {
  if (!energy) return null;
  const match = energy.match(/^(\d+|-)(?:\(([A-Z]+)\))?$/);
  ...
  if (match) {
    ...
    return (
      <span className="flex items-center text-[11px] font-bold text-white leading-none">
        {total !== '-' && <span className="mr-1">{total}</span>}
        {colors.split('').map((c, i) => {
           let dotBg = "bg-gray-400";
           if (c === 'R') dotBg = "bg-[#E62020] border border-[#FF6B6B]";
           else if (c === 'B' || c === 'U') dotBg = "bg-[#0A5BC4] border border-[#4D94FF]";
           else if (c === 'G') dotBg = "bg-[#1C8A33] border border-[#40CC5E]";
           else if (c === 'Y') dotBg = "bg-[#D9A100] border border-[#FFCF3D]";
           else if (c === 'K' || c === 'W') {
             // Handle K and W similar to before, K is black, W is white
             dotBg = c === 'K' ? "bg-zinc-800 border border-zinc-500/50" : "bg-white border border-gray-300";
           }
           
           return (
             <span key={i} className={`inline-flex items-center justify-center w-3 h-3 rounded-full ${dotBg} shadow-[0_1px_2px_rgba(0,0,0,0.5)] mr-0.5`} title={c} />
           );
        })}
      </span>
    );
  ...
  return <span className="font-bold text-white text-[10px]">{energy}</span>;
};

const renderSkillText = (text: string) => {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('[') && part.endsWith(']')) {
      ...
      // Check if it is a colorless energy cost like [one], [two], [1], [2], etc.
      ...
      if (numMap[lower] !== undefined || /^\d+$/.test(lower)) {
        const val = numMap[lower] || lower;
        return (
          <span key={index} className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-white border border-gray-400 text-black text-[9px] font-black shadow-[0_1px_2px_rgba(0,0,0,0.4)] mx-0.5 align-middle select-none" style={{ transform: 'translateY(-1px)' }}>
            {val}
          </span>
        );
      }
      
      let bgClass = "bg-red-600 border border-red-500 text-white";
      let txtClass = "text-white";
      
      if (lower.includes('auto')) {
        bgClass = "bg-[#0A417C] border border-[#1E67C1]";
      } else if (lower.includes('activate') || lower.includes('awaken') || lower.includes('wish')) {
        bgClass = "bg-[#D9A100] border border-[#FFCF3D]";
        txtClass = "text-black drop-shadow-[0_1px_0_rgba(255,255,255,0.4)]";
      } else if (lower.includes('counter')) {
        bgClass = "bg-[#1E7C1E] border border-[#2DB82D]";
      }
      
      return (
        <span key={index} className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-black mr-1 uppercase tracking-wider align-middle shadow-sm leading-none bg-opacity-90 ${bgClass} ${txtClass}`}>
          {content}
        </span>
      );
    }
      ...
      <span key={index} className="align-middle inline-block">
        {textParts.map((tPart, j) => {
            if (/^(?:\{[a-zA-Z0-9]+\})+\s*\d*$/.test(tPart)) {
              const marks = [...tPart.matchAll(/\{([a-zA-Z0-9]+)\}/g)];
              const trailingNumMatch = tPart.match(/\}\s*(\d+)$/);
              const trailingNum = trailingNumMatch ? trailingNumMatch[1] : null;
              
              const allValid = marks.length > 0 && marks.every(m => {
                const colorChar = m[1].toUpperCase();
                const validColors = ['R', 'B', 'U', 'G', 'Y', 'K', 'BK', 'W', 'WHITE', 'BLACK'];
                return validColors.includes(colorChar) || /^\d+$/.test(colorChar);
              });
                ...
                return (
                  <span key={j} className="inline-flex items-center align-middle mx-1" style={{ transform: 'translateY(-1px)' }}>
                    {marks.map((m, k) => {
                      ...
                      else if (colorChar === 'B' || colorChar === 'U') dotBg = "bg-[#0A5BC4] border border-[#4D94FF]";
                      else if (colorChar === 'G') dotBg = "bg-[#1C8A33] border border-[#40CC5E]";
                      else if (colorChar === 'Y') dotBg = "bg-[#D9A100] border border-[#FFCF3D]";
                      else if (colorChar === 'K' || colorChar === 'BK' || colorChar === 'BLACK') dotBg = "bg-zinc-800 border border-zinc-500/50";
                      else if (colorChar === 'W' || colorChar === 'WHITE') dotBg = "bg-white border border-gray-300";
                      else if (/^\d+$/.test(colorChar)) {
                        dotBg = "bg-white border border-gray-300";
                        innerText = colorChar;
                      }
                      
                      return (
                        <span key={k} className={`inline-flex items-center justify-center w-3.5 h-3.5 rounded-full ${dotBg} shadow-[0_1px_2px_rgba(0,0,0,0.5)] mr-0.5`} title={colorChar}>
                          {innerText && <span className="text-black text-[9px] font-black leading-none">{innerText}</span>}
                        </span>
                      );
                    })}
                    {trailingNum && (
                      <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-white border border-gray-300 text-black text-[9px] font-black shadow-[0_1px_2px_rgba(0,0,0,0.5)] mr-0.5" style={{ lineHeight: 1 }}>
                        {trailingNum}
                      </span>
                    )}
                  </span>
...
const isHorizontalFormat = (card: Card) => card.type === 'Playmat';
...
export default function TrackerApp() {
  ...
  const filteredCards = cards.filter(card => {
    ...
    const cardCharacters = card.character?.split(/[\/\-]/).map(s => s.trim()) || [];
    const matchesCharacter = filters.characters.length === 0 || filters.characters.some(c => cardCharacters.includes(c));

    const cardTraits = (card.specialTrait || card.traits)?.split(/[\/\-]/).map(s => s.trim()) || [];
    const matchesTrait = filters.traits.length === 0 || filters.traits.some(t => cardTraits.includes(t));

    const cardEras = card.era?.split(/[\/\-]/).map(s => s.trim()) || [];
    const matchesEra = filters.eras.length === 0 || filters.eras.some(e => cardEras.includes(e));
    ...
    return matchesSearch && matchesExpansion && matchesRarity && matchesColor && matchesType && matchesLegal && matchesCharacter && matchesTrait && matchesEra;
  }).sort((a, b) => {
    ...
    if (filters.expansion !== 'Todos') {
      ...
      // Special override: BT1-111 cards in BT18 should go to the end of the list
      if (filters.expansion === 'BT18') {
        const aIsBt1_111 = aBase.startsWith('BT1-111');
        const bIsBt1_111 = bBase.startsWith('BT1-111');
        if (aIsBt1_111 && !bIsBt1_111) return 1;
        if (!aIsBt1_111 && bIsBt1_111) return -1;
      }
  ...
  const availableOptions = useMemo(() => {
    ...
    const characters = Array.from(new Set(expansionCards.flatMap(c => c.character?.split(/[\/\-]/).map(s => s.trim()) || []).filter(Boolean))).sort();
    const traits = Array.from(new Set(expansionCards.flatMap(c => (c.specialTrait || c.traits)?.split(/[\/\-]/).map(s => s.trim()) || []).filter(Boolean))).sort();
    const eras = Array.from(new Set(expansionCards.flatMap(c => c.era?.split(/[\/\-]/).map(s => s.trim()) || []).filter(Boolean))).sort();

    return { colors, rarities, types, characters, traits, eras };
  ...
  useEffect(() => {
    ...
    if (filters.characters.length > 0) {
      const validSelectedCharacters = filters.characters.filter(c => availableOptions.characters.includes(c));
      if (validSelectedCharacters.length !== filters.characters.length) {
        setFilters(prev => ({ ...prev, characters: validSelectedCharacters }));
      }
    }
    if (filters.traits.length > 0) {
      const validSelectedTraits = filters.traits.filter(t => availableOptions.traits.includes(t));
      if (validSelectedTraits.length !== filters.traits.length) {
        setFilters(prev => ({ ...prev, traits: validSelectedTraits }));
      }
    }
    if (filters.eras.length > 0) {
      const validSelectedEras = filters.eras.filter(e => availableOptions.eras.includes(e));
      if (validSelectedEras.length !== filters.eras.length) {
        setFilters(prev => ({ ...prev, eras: validSelectedEras }));
      }
    }
  }, [availableOptions, filters.colors.length, filters.rarities.length, filters.types.length, filters.characters.length, filters.traits.length, filters.eras.length]);
  ...
  useEffect(() => {
    const loadCards = async () => {
      ...
      let parsedCards: (Card & { baseIndex?: number })[] = combinedData.split('\n').filter(line => line.trim()).map((line, i) => {
        ...
        let backPower, backCharacter, backEra, backSpecialTrait, backSkill;
        
        const splitField = (field: string | undefined): [string | undefined, string | undefined] => {
          if (!field) return [undefined, undefined];
          const splitParts = field.split('//').map(s => s.trim());
          if (splitParts.length > 1) return [splitParts[0], splitParts[1]];
          return [field, undefined];
        };

        [power, backPower] = splitField(power);
        [character, backCharacter] = splitField(character);
        [specialTrait, backSpecialTrait] = splitField(specialTrait);
        [era, backEra] = splitField(era);
        [skill, backSkill] = splitField(skill);
        ...
        return {
          ...
          backSpecialTrait: backSpecialTrait,
          backEra: backEra,
          backSkill: backSkill,
      ...
      const baseCardMap = new Map<string, Card>();
      ...
      parsedCards = parsedCards.map(card => {
        ...
        if (card.cardNumber.includes('_') && baseCardMap.has(base)) {
          const baseCard = baseCardMap.get(base)!;
          if (!card.power) card.power = baseCard.power;
          if (!card.energy) card.energy = baseCard.energy;
          if (!card.comboEnergy) card.comboEnergy = baseCard.comboEnergy;
          if (!card.comboPower) card.comboPower = baseCard.comboPower;
          if (!card.character) card.character = baseCard.character;
          if (!card.specialTrait) card.specialTrait = baseCard.specialTrait;
          if (!card.era) card.era = baseCard.era;
          if (!card.skill) card.skill = baseCard.skill;
          if (!card.backPower) card.backPower = baseCard.backPower;
          if (!card.backCharacter) card.backCharacter = baseCard.backCharacter;
          if (!card.backSpecialTrait) card.backSpecialTrait = baseCard.backSpecialTrait;
          if (!card.backEra) card.backEra = baseCard.backEra;
          if (!card.backSkill) card.backSkill = baseCard.backSkill;
        }
        return card;
      });
    ...
    <div className="min-h-screen bg-[#121212] text-gray-100 pb-32 font-sans">
                        ...
                        <motion.button
                          ...
                          onClick={() => {
                            ...
                            if (cat.id === 'promos') {
                               ...
                               setFilters({...filters, expansion: gameType === 'fusion' ? 'ENM_FW' : 'ENM'});
                            } else {
                  ...
                  (() => {
                          ...
                          // LEVEL 2 LIST OF SETS
                            ...
                            if (isColeccionismo) {
                              ...
                              if (parent) {
                                activeGroup = { items: parent.subItems || [parent] };
                              }
                            } else {
              ...
              <div className="space-y-6">
                ...
                {filteredCards.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <button 
                      onClick={() => {
                        setFilters({ rarities: [], colors: [], expansion: filters.expansion, types: [], legalStatus: [], alternatives: DEFAULT_ALTERNATIVES, traits: [], characters: [], eras: [], owned: 'all' });
                        setSearchQuery('');
                      }}
      ...
      <AnimatePresence mode="wait">
        {selectedCard && (
          ...
          >
            <div className="min-h-screen flex items-center justify-center p-3 sm:p-6 pb-24 relative">
                      ...
                      <div className="bg-black/50 p-3 sm:p-5 rounded-3xl border border-white/10 backdrop-blur-2xl shadow-2xl flex flex-col gap-5 text-left w-full">
                        <div className={`flex gap-3 sm:gap-4 ${isHorizontalFormat(selectedCard) ? 'flex-col items-center' : ''}`}>
                          ...
                          <div className="flex-1 min-w-0 pt-1">
                            ...
                            {( (isFlipped && selectedCard.backPower ? selectedCard.backPower : selectedCard.power) || selectedCard.energy || selectedCard.comboEnergy || selectedCard.comboPower) && (
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3 bg-white/5 p-2 rounded-xl">
                                {(isFlipped && selectedCard.backPower ? selectedCard.backPower : selectedCard.power) && (
                                  <div>
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Power</p>
                                    <p className="font-bold text-white text-[10px]">{(isFlipped && selectedCard.backPower ? selectedCard.backPower : selectedCard.power)}</p>
                                  </div>
                                )}
                                {selectedCard.energy && (
                                  <div>
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Energy</p>
                                    {renderEnergyCost(selectedCard.energy)}
                                  </div>
                                )}
                                {selectedCard.comboEnergy && (
                                  <div>
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">C.Energy</p>
                                    <p className="font-bold text-white text-[10px]">{selectedCard.comboEnergy}</p>
                                  </div>
                                )}
                                {selectedCard.comboPower && (
                                  <div>
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">C.Power</p>
                                    <p className="font-bold text-white text-[10px]">{selectedCard.comboPower}</p>
                                  </div>
                                )}
                              </div>
                            )}

                            {( (isFlipped && selectedCard.backCharacter ? selectedCard.backCharacter : selectedCard.character) || (isFlipped && selectedCard.backSpecialTrait ? selectedCard.backSpecialTrait : selectedCard.specialTrait) || (isFlipped && selectedCard.backEra ? selectedCard.backEra : selectedCard.era) ) && (
                              <div className="flex flex-col gap-y-2 mb-3 border-t border-white/10 pt-3">
                                {(isFlipped && selectedCard.backCharacter ? selectedCard.backCharacter : selectedCard.character) && (
                                  <div className="w-full">
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Character</p>
                                    <p className="font-bold text-white text-[10px] whitespace-normal break-words">{(isFlipped && selectedCard.backCharacter ? selectedCard.backCharacter : selectedCard.character)}</p>
                                  </div>
                                )}
                                {(isFlipped && selectedCard.backSpecialTrait ? selectedCard.backSpecialTrait : selectedCard.specialTrait) && (
                                  <div className="w-full">
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Trait</p>
                                    <p className="font-bold text-white text-[10px] whitespace-normal break-words">{(isFlipped && selectedCard.backSpecialTrait ? selectedCard.backSpecialTrait : selectedCard.specialTrait)}</p>
                                  </div>
                                )}
                                {(isFlipped && selectedCard.backEra ? selectedCard.backEra : selectedCard.era) && (
                                  <div className="w-full">
                                    <p className="text-[8px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Era</p>
                                    <p className="font-bold text-white text-[10px] whitespace-normal break-words">{(isFlipped && selectedCard.backEra ? selectedCard.backEra : selectedCard.era)}</p>
                                  </div>
                                )}
                              </div>
                            )}
                        ...
                        {(isFlipped && selectedCard.backSkill ? selectedCard.backSkill : selectedCard.skill) && (
                          <div className="p-4 bg-white/5 rounded-2xl border border-white/10 mt-1">
                            <p className="text-[9px] text-gray-400 uppercase font-black tracking-widest mb-1.5 leading-none">Skill</p>
                            <div className="text-[11px] text-gray-300 font-medium leading-relaxed space-y-2">
                              {(isFlipped && selectedCard.backSkill ? selectedCard.backSkill : selectedCard.skill)!.split('|').map((line, i) => (
                                <p key={i} className="flex flex-wrap items-center gap-y-1">
                                  {renderSkillText(line.trim())}
                                </p>
                              ))}
                            </div>
                          </div>
          ...
          <>
            ...
            >
              ...
              <div className="flex justify-between items-center mb-6 sticky top-0 bg-[#1E1E1E] z-10 py-2">
                ...
                <button 
                  onClick={() => setFilters({ rarities: [], colors: [], expansion: filters.expansion, types: [], legalStatus: [], alternatives: DEFAULT_ALTERNATIVES, traits: [], characters: [], eras: [], owned: 'all' })}
              ...
              <div className="space-y-6">
                ...
                {availableOptions.colors.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <MultiSelect
                      options={availableOptions.colors.map(c => ({ value: c, label: (t as any).colorNames?.[c] || c }))}
                      values={filters.colors}
                      onChange={(newColors) => setFilters({...filters, colors: newColors})}
                      placeholder={lang === 'es' ? 'Seleccionar colores...' : 'Select colors...'}
                      searchPlaceholder={lang === 'es' ? 'Buscar colores...' : 'Search colors...'}
                    />
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <MultiSelect
                    ...
                    searchPlaceholder={lang === 'es' ? 'Buscar rarezas...' : 'Search rarities...'}
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <MultiSelect
                    options={availableOptions.types.map(typeVal => ({ value: typeVal, label: (t as any).typeNames?.[typeVal] || typeVal }))}
                    values={filters.types}
                    onChange={(newTypes) => setFilters({...filters, types: newTypes})}
                    placeholder={lang === 'es' ? 'Seleccionar tipos...' : 'Select types...'}
                    searchPlaceholder={lang === 'es' ? 'Buscar tipos...' : 'Search types...'}
                  />
                </div>
                ...
                {availableOptions.characters?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{lang === 'es' ? 'Personaje' : 'Character'}</p>
                      <button 
                        onClick={() => setFilters({...filters, characters: []})}
                        className="text-[10px] font-black text-orange-500 uppercase hover:underline"
                      >
                    ...
                    <MultiSelect
                      options={availableOptions.characters.map(c => ({ value: c, label: c }))}
                      values={filters.characters}
                      onChange={(newCharacters) => setFilters({...filters, characters: newCharacters})}
                      placeholder={lang === 'es' ? 'Seleccionar personaje...' : 'Select character...'}
                      searchPlaceholder={lang === 'es' ? 'Buscar personaje...' : 'Search character...'}
                    />
                  </div>
                )}

                {availableOptions.traits?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{lang === 'es' ? 'Rasgo especial' : 'Special Trait'}</p>
                      <button 
                        onClick={() => setFilters({...filters, traits: []})}
                        className="text-[10px] font-black text-orange-500 uppercase hover:underline"
                      >
                        {lang === 'es' ? 'LIMPIAR' : 'CLEAR'}
                      </button>
                    ...
                    <MultiSelect
                      options={availableOptions.traits.map(tOption => ({ value: tOption, label: tOption }))}
                      values={filters.traits}
                      onChange={(newTraits) => setFilters({...filters, traits: newTraits})}
                      placeholder={lang === 'es' ? 'Seleccionar rasgo...' : 'Select trait...'}
                      searchPlaceholder={lang === 'es' ? 'Buscar rasgo...' : 'Search trait...'}
                    />
                  </div>
                )}

                {availableOptions.eras?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Era</p>
                      <button 
                        onClick={() => setFilters({...filters, eras: []})}
                        className="text-[10px] font-black text-orange-500 uppercase hover:underline"
                      >
                        {lang === 'es' ? 'LIMPIAR' : 'CLEAR'}
                      </button>
                    ...
                    <MultiSelect
                      options={availableOptions.eras.map(e => ({ value: e, label: e }))}
                      values={filters.eras}
                      onChange={(newEras) => setFilters({...filters, eras: newEras})}
                      placeholder={lang === 'es' ? 'Seleccionar era...' : 'Select era...'}
                      searchPlaceholder={lang === 'es' ? 'Buscar era...' : 'Search era...'}
                    />
                  </div>
                )}
